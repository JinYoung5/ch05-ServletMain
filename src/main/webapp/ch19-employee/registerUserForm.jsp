...
$(document).ready(function(){
	//중복체크 결과 : 0 -> 아이디 중복체크 미실시, 아이디중복. 1-> 아이디 미중복
	let count = 0;
	//이벤트 연결
		...
		$.ajax({
			...
			success:function(param){
				if(param.result == 'idDuplicated'){
					count = 0;
					$('#id_signed').text('아이디 중복').css('color','blue');
					$('#id').val('').focus();
				}else if(param.result == 'idNotFound'){
					count = 1;
					$('#id_signed').text('사용 가능 아이디').css('color','gray');
				}else{
					count = 0;
	...
	<form action="registerUser.jsp" method="post" id="register_form">
		<ul>
			...
			<li>
				...
				<input type="text" name="job" id="job" maxlength="10">
			</li>
		</ul>
